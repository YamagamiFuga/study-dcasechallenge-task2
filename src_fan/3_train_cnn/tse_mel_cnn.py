...
class ResNetFrontend(nn.Module):
    ...
    def __init__(self, n_classes=5, emb_dim=128):
        super().__init__()
        ...
        # 特徴ベクトル変換
        # ResNetの出力は512次元なので、emb_dim(128)次元に変換する全結合層を追加
    ...
    def forward(self, x):  # (B,1,n_mels,T)
        ...
        # 512次元の特徴ベクトルをemb_dim(128)次元に変換
        ...
        zの値は例えば、[[0.5, -1.2, 0.3, ..., 0.8],
                        ...
                        ...,
                        [1.0, -0.5, 0.2, ..., 0.6]]
        みたいな感じで、各サンプルに対して128次元の特徴ベクトルが出力される
        """
...
class TrainDataset(Dataset):
    def __init__(self, noisy_paths, enhanced_paths, processor):
        self.pairs = list(zip(noisy_paths, enhanced_paths))
    ...
    def __getitem__(self, idx):
        # 同じ元音声に対する Noisy / TSE をペアで持つ
        noisy_path, enh_path = self.pairs[idx]

        # 50%の確率でNoisyかEnhancedを使う
...
# 学習用のDatasetを作成する NoisyとEnhancedをペアで扱うDataset
train_dataset = TrainDataset(
    noisy_paths=train_wav_path,
    enhanced_paths=train_denoised_wav_path,
    processor=processor
)
...
model.load_state_dict(torch.load("cnn_frontend.pth", map_location="cpu"))
...
with torch.no_grad():
    for path in tqdm(test_wav_path, desc="Testing"):
        # メルスペクトログラム
...
for path, score in tqdm(zip(test_wav_path, test_scores), desc="Categorizing Scores"):
